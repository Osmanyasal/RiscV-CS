...
task print_cpu_dashboard();
    ...
    // ------------------------------------------------------------
    // Simple Disassembler & Immediate Selector
    // ------------------------------------------------------------
    ...
    if (dut.control_opcode == 7'b0110011) begin // R-Type
        ...
        $display("\tBinary Split  : %b_%b_%b_%b_%b_%b", 
            dut.inst_out[31:25], dut.inst_out[24:20], dut.inst_out[19:15], 
            dut.inst_out[14:12], dut.inst_out[11:7],  dut.inst_out[6:0]);
        $display("\tField Map     : [imm11:5] [rs2] [rs1] [f3] [imm4:0] [opcode]");

    end else if (dut.control_opcode == 7'b1100011) begin // B-Type
        $display("\tBinary Split  : %b_%b_%b_%b_%b_%b_%b_%b",
            dut.inst_out[31],        // imm[12]
            dut.inst_out[30:25],     // imm[10:5]
            dut.inst_out[24:20],     // rs2
            dut.inst_out[19:15],     // rs1
            dut.inst_out[14:12],     // f3
            dut.inst_out[11:8],      // imm[4:1]
            dut.inst_out[7],         // imm[11]
            dut.inst_out[6:0]);      // opcode

        $display("\tField Map     : [imm12] [imm10:5] [rs2] [rs1] [f3] [imm4:1] [imm11] [opcode]");

    end else begin // I-Type, U-Type, J-Type default view
    ...
    $display("\tReg Access    : RD1(x%0d)=0x%h, RD2(x%0d)=0x%h", 
              dut.control_rs1, dut.regfile_rd1, 
              dut.control_rs2, dut.regfile_rd2);
    ...
    // LOAD
    if (dut.control_opcode == 7'b0000011) begin
        $display("\tAction        : MEM_READ  [%0d] -> 0x%h", 
                 signed'(dut.alu_out_addr), dut.data_mem_rd);

        if (dut.control_regfile_we)
            $display("\tAction        : WRITE REG x%0d <= 0x%h", 
                     signed'(dut.control_rd), dut.data_mem_rd);
    end

    // STORE
    else if (dut.control_opcode == 7'b0100011 && dut.control_data_mem_we) begin
        $display("\tAction        : MEM_WRITE [%0d] <= 0x%h", 
                 signed'(dut.alu_out_addr), dut._data_mem_wr);
    end

    // BRANCH
    else if (dut.control_opcode == 7'b1100011) begin
        $display("\tAction        : BRANCH Target = %0d (PC + %0d)", 
                 signed'(dut.pc_current) + signed'(current_imm), signed'(current_imm));
    end

    // Default ALU writeback
    else begin
        if (dut.control_regfile_we)
            $display("\tAction        : ALU_RESULT 0x%h -> x%0d", 
                     dut.alu_out_addr, signed'(dut.control_rd));
        else
            $display("\tAction        : NO WRITEBACK");
    end
